...
import { Plus, Filter, CheckCircle2, Circle, Clock, Calendar as CalendarIcon, MoreVertical } from 'lucide-react';
  ...
  const TaskItem: React.FC<{ task: Task }> = ({ task }) => (
    ...
    >
      <button 
        ...
        className="mt-1 text-gray-300 dark:text-gray-600 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
      >
        <Circle size={24} strokeWidth={1.5} />
      </button>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 mb-1.5">
          <h3 className="font-medium text-gray-900 dark:text-gray-100 truncate text-base">{task.title}</h3>
